...
module.exports.run = async (client, message, args) => {
    ...
    if (!message.guild.me.permissions.has("KICK_MEMBERS")) return message.reply("The bot has no permissions, please contact the bot developer.");
    ...
    if (kickUser.permissions.has("KICK_MEMBERS")) return message.reply("You can't kick this person.");
    ...
    var embed = new discord.MessageEmbed()
        ...
        **User got kicked by:** ${message.author}
        **Reason:** ${reason}`)
        .setTimestamp();
    ...
    message.channel.send({ embeds: [embedPrompt] }).then(async msg => {
        ...
        msg.awaitReactions({ filter, max: 1, time: time }).then(collected => {
            ...
            if (emojiDetails.emoji.name === "✅") {
                ...
                message.channel.send("Kick canceled").then(msg => {
                    message.delete()
